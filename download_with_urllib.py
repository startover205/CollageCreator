...
def parse_page(url):
    ...
    if page_content:
        ...
        if len(link_list) == 0:
            ...
            print(f'get {len(link_list)} links from page {url}')
...
def download_images(main_keyword, supplemented_keywords, download_dir):
    ...
    """
    image_links = set()
    print('Process ID - {0} Main keyword: {1}'.format(os.getpid(), main_keyword))
...
def download(path, main_keywords, supplemented_keywords):
    if len(main_keywords) == 0:
        raise Exception("main keywords can not be empty")
    if len(supplemented_keywords) == 0:
        raise Exception("supplemented keywords can not be empty")

    # download with single process
    for i in range(len(main_keywords)):
        download_images(main_keywords[i], supplemented_keywords, path)

    # download with multiple process
    # p = Pool()  # number of process is the number of cores of your CPU
    # for i in range(len(main_keywords)):
    #     p.apply_async(download_images, args=(main_keywords[i], supplemented_keywords, path))
    # p.close()
    # p.join()
...
if __name__ == '__main__':
    ...
    # must not be empty
    main = ['desk', 'chair', 'hand', 'foot', 'nose', 'eye']
    supplemented = ['cartoon']

    download(download_dir, main, supplemented)
